...
export const postAssay = async (credentials: any) => {
  const option = {
    ...
    headers: {
      "Content-Type":
        "multipart/form-data; boundary=----WebKitFormBoundary6kFlpsUUnoYbwXnV",
    },
    body: credentials,
  };
...
export const getAssaySearch = async (credentials: any) => {
  const option = {
    method: "POST",
    headers: {
      "Content-Type": "application/json;charset=UTF-8",
  ...
  return await response(`assay/search/`, option);
};
